...
class DeepQNetwork:
    def __init__(self, width, height, numActions, baseDir, learningRate, modelFile, saveModelFrequency, targetModelUpdateFrequency, evalEpsilon):
        ...
        with tf.device(None):
          ...
          self.train_step = self.buildGravesRMSPropOptimizer(learningRate, .95, .01, 0)
    ...
    # from github.com/Jabberwockyll/deep_rl_ale
    def buildGravesRMSPropOptimizer(self, learning_rate, rmsprop_decay, rmsprop_constant, gradient_clip):
        ...
        with tf.name_scope('rmsprop'):
            ...
            rms = [tf.sqrt(avg_grad_pair[1] - tf.square(avg_grad_pair[0]) + rmsprop_constant) for avg_grad_pair in zip(avg_grads, avg_square_grads)]
